...
export const getCampignDetails = async (campaignId: string): Promise<ICampaignDetail> => {
  return await get(`/campaign/detail/${campaignId}`);
};

export const getContactOfCampaign = async (campaingId: string): Promise<ICampaignRecipient> => {
  return await get(`/campaign/contactInfo/${campaingId}`);
};

export const runCampaignNow = async (campaignId: string): Promise<ICampaignStartRes> => {
...
export const deleteCampaignApi = async (campaignId: string, workspaceId: string) => {
