...
import EditWorkspaceModal from './components/EditWorkspaceForm';
...
import type { IAddMembersDto, IWorkspaceFormData, IWorkspaceRow } from './types';
...
export default function Workspace() {
  ...
  const { workspaces: rows, loading } = useSelector((state: RootState) => state.adminWorkspace);
      ...
      <Grid container spacing={2} mb={3}>
        <Grid size={{ xs: 12, md: 4 }}>
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField {...(params as { size: string })} size="medium" label="Workspace" />
            )}
          />
        </Grid>
