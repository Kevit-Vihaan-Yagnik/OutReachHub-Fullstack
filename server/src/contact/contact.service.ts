...
export class ContactService {
  ...
  async createContact(
    ...
    // Collect all emails from DTO
    const emails = dto.contacts.map((c) => c.contactInfo.email);

    // Check if any email already exists in this workspace
    const existingContacts = await this.ContactModel.find({
      workspaceId,
      'contactInfo.email': { $in: emails },
    });

    if (existingContacts.length > 0) {
      const existingEmails = existingContacts.map((c) => c.contactInfo.email);
      throw new HttpException(
        `Contact(s) with email(s) ${existingEmails.join(', ')} already exist in this workspace`,
        400,
      );
    }
    ...
    return {
      message: `${createdContact.length} contact(s) added successfully`,
      data: createdContact,
    };
  }
