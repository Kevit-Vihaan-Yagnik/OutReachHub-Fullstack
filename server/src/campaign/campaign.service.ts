...
export class CampaignService {
    constructor(
        ...
        @InjectModel(CampaignRecepient.name) private CampaignRecepientModel: Model<CampaignRecepient>,
    ...
    async prepareRecepientsforCampaign(campaignId: string) {
        ...
        if (!campaign || campaign.isDeleted) throw new HttpException("Campaign not found", 404);

        const tags = campaign.tags || [];

        if (!tags.length) {
            await this.CampaignRecepientModel.deleteMany({ campaignId: campaign._id });
        ...
        if (docs.length) {
            try {
                await this.CampaignRecepientModel.deleteMany({ campaignId: campaignId });
    ...
    async runNow(campaignId: string, userId: string) {
        ...
        return { message: 'campaign started', campaignId, audienceSize: campaign.audienceSize ?? 0 }
    }

    async sendCampaign(campaignId: string) {
        const campaign = await this.CampaignModel.findById(campaignId).populate("templateId");
        ...
        const template = await this.MessageTemplateModel.findById(campaign.templateId);
    ...
    async mailGone() {
        const message = await this.mailService.sendCampaignEmail("yagnikvihaan5@gmail.com", "hello", "somecontent");
